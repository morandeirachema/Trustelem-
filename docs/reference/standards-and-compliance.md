...
> - **Purpose:** for each protocol standard, regulation and assurance scheme that applies to the
>   Trustelem, Bastion and Access Manager design, the clause, what it requires, and whether the
>   design meets it.
> - **Audience:** the PAM architect, the security and compliance officers, and auditors.
> - **Verified:** 2026-09-23 for the standards and regulatory texts and the vendor assurance
>   pages; reviewed 2026-09-24 (the PCI DSS requirement texts were not re-read, see
>   [section 2](#2-regulatory-drivers)).
> - **Sources:** the standard, regulation or vendor page linked in each row;
>   [Bastion Administration Guide](https://pam.wallix.one/documentation/admin-doc/bastion_en_administration_guide.pdf),
...
Each row gives the citation, what the clause requires, and how the design meets it or does
not. Items marked *uncertain* or *gap* are not confirmed by a primary source; their gap IDs
refer to the [register](open-questions-and-gaps.md).

## 1. Protocol standards behind the Trustelem integration

The design follows the RADIUS, SAML, OIDC, LDAP and authenticator standards, with two weak
points: RADIUS over UDP without a confirmed Message-Authenticator, and a native path that cannot
be phishing-resistant.

### 1.1 RADIUS (native client path: Bastion to Trustelem Connect)
...
### 1.2 SAML 2.0 (web path: Trustelem to Access Manager and Bastion)

| Reference | Requirement | Design fit |
|-----------|-------------|------------|
| [saml-core-2.0-os](https://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf) 1.3.3 and 2.5.1.2 | UTC time values; NotBefore and NotOnOrAfter validity window; the spec sets no numeric skew tolerance | NTP on every node is a hard prerequisite; Access Manager's skew tolerance is not published (*uncertain*, gap A4); Trustelem: "Verify the time on Access Manager: SAML assertion are valid for a short period" |
| [saml-profiles-2.0-os](https://docs.oasis-open.org/security/saml/v2.0/saml-profiles-2.0-os.pdf) 4.1 Web Browser SSO | With HTTP-POST the assertion must be signed; the SP must prevent replay | Trustelem signs with a per-application certificate; Access Manager requires Signed Response and Signed Assertion; replay cache on AM is not documented (*uncertain*, gap A4); WSA-2026-07-0002 (SAML SP bypass before 5.1.10, 5.2.7, 6.0.4) shows why the patch level matters |
...
### 1.3 OpenID Connect (alternative web path)
...
| [RFC 9700 / BCP 240](https://www.rfc-editor.org/rfc/rfc9700.html) | PKCE for code flows, exact redirect URI matching, no implicit grant | *Gap* (A5): PKCE support on Bastion, Access Manager and the Trustelem provider is not documented; Trustelem still offers the implicit flow, so do not enable it |
...
### 1.4 Directory and authenticator standards
...
The design supports the MFA and privileged-access clauses of every driver below. The exceptions
recorded in the rows are ANSSI PA-022 R6 and R37 (no ANSSI qualification, no qualified X.509
authentication), ANSSI PG-078 R39 for phone-based factors, and the PCI DSS exceptions to
document for the break-glass account and TOTP windows.
...
Bastion 12.0.14 holds a BSI BSZ certificate; no Common Criteria certificate and no SecNumCloud
qualification of Trustelem were found.
...
| Common Criteria | none found (*gap* S3) |
...
| Trustelem hosting | "Hosted in European Data Centers" ([product page](https://www.wallix.com/products/idaas/)); provider, country and DR set-up not published (*gap* T9) |

## 4. Threat model (MITRE ATT&CK)

MFA and the Bastion proxies cover the main techniques; push fatigue on the RADIUS path is the
residual exposure.
...
| [T1621 MFA Request Generation](https://attack.mitre.org/techniques/T1621/) (push fatigue) | M1036, M1032 ("more secure 2FA/MFA mechanisms in replacement of simple push"), M1017 | the RADIUS push-wait path is exposed; mitigate with zone rules, TOTP or password+code on RADIUS, passkeys on the web path, and SIEM detection of repeated requests; number matching in WALLIX Authenticator is not documented (*gap* T7) |

## 5. Questions for WALLIX raised by this mapping

The open questions below are tracked in the [register](open-questions-and-gaps.md) and asked in
the [vendor meeting script](vendor-meeting-script.md).

| # | Question | Gap |
|---|----------|-----|
| 1 | Message-Authenticator support on the Bastion RADIUS client and on Trustelem Connect, and a statement on CVE-2024-3596 | T4 |
| 2 | RadSec or RFC 9765 roadmap for Trustelem Connect | T4 |
| 3 | PKCE and exact redirect URI matching on the Bastion and Access Manager OIDC clients and on the Trustelem provider | A5 |
| 4 | Access Manager SAML clock-skew tolerance and assertion replay cache | A4 |
| 5 | Number matching or push rate limiting in WALLIX Authenticator | T7 |
| 6 | Certification coverage of Bastion 12.3 and 12.4 versus the certified 12.0.14; ANSSI qualification status | S3 |
| 7 | Trustelem hosting provider, location, SecNumCloud or HDS status, ISO 27001 certificate number and statement of applicability | T9 |
