...
> - **Purpose:** what each component logs, how it reaches the SIEM, the Bastion and Trustelem
>   event formats, the keys that correlate them, and the detection rules for the MFA design.
> - **Audience:** SIEM engineers, the security operations team and the PAM operations team.
> - **Verified:** 2026-09-24, against the Bastion 12.4.3 SIEM Logs, Operation, Deployment and
>   Auditor Guides, the Bastion 12.0.25 SIEM Logs Guide (version differences) and the Access
>   Manager 6.0.5 Administration and Deployment Guides.
> - **Sources:** Bastion and Access Manager customer guides behind the [doc.wallix.com](https://doc.wallix.com/) login,
>   [WALLIX Splunk add-on](https://github.com/wallix/Splunk-add-on) (README and `default/props.conf`),
>   [Sekoia WALLIX Bastion intake](https://docs.sekoia.com/integration/categories/iam/wallix/),
...
The guides linked to <https://doc.wallix.com/> are WALLIX customer documentation behind the
doc.wallix.com login (WALLIX Trustelem SSO); section numbers are given so they can be found.
Older Access Manager 5.2 statements are labelled as such. The SIEM Logs Guide "lists log
examples and is not exhaustive" (1).

## 1. Sources and transport

Trustelem pushes its logs through Trustelem Connect. Each Bastion node sends syslog itself.
Access Manager keeps local files, and its forwarding is not confirmed (gap A7).

| Source | Transport | Format | Retention at source | Details |
|--------|-----------|--------|---------------------|---------|
| Trustelem | push through Trustelem Connect to an on-premise target, every 30 s, queued on error | JSON (recommended) or syslog | 30 days through the API; console retention not documented | [1.1](#11-trustelem) |
| Bastion nodes | **System > SIEM integration**, per node | syslog, key=value inside a bracketed event tag ([section 2](#2-bastion-event-format)) | local files, up to 365 days | [1.2](#12-bastion-nodes) |
| Access Manager nodes | not confirmed (gap A7) | Apache-style access log, application logs | files for the "ongoing day (static 24h)", downloaded as an archive | [1.3](#13-access-manager-log-files-and-forwarding) |
| Agents (ADConnect, Trustelem Connect) | OS logs only | | | |

### 1.1 Trustelem

- The push goes through Trustelem Connect to an on-premise target every 30 s, with a queue on
  error. JSON is the recommended format; syslog is the alternative
  ([Trustelem on-premise SIEM](https://trustelem-doc.wallix.com/books/trustelem-administration/page/on-premise-siem);
  set-up steps in [07 Operations](../trustelem/07-operations.md), section 2).
- The API returns 30 days: "List all the logs of the 30 previous days"
  ([Trustelem API](https://trustelem-doc.wallix.com/books/trustelem-administration/page/api)).
  The console retention is not documented (gap T14).

### 1.2 Bastion nodes

Configuration ([Bastion 12.4.3 Operation Guide](https://doc.wallix.com/) 13.6; same text in
12.0.25, 13.5):

- **System > SIEM integration** holds one row per destination: address, "transmission protocol
  (UDP, TCP, or TLS)", port, "log format (standard RFC 5424 or RFC 3164)".
- With RFC 3164, "the timestamp can be in RFC 3164 or ISO (YYYY-MM-DD-HH:MM:SS±TZ)".
- "Using TLS is strongly recommended" (PEM file with the CA chain).
- "This feature is only available when the SIEM feature is associated with the license key."
- Each filter of the SIEM Logs Guide can be enabled or disabled. "WALLIX recommends that you
  enable all logs and that you filter out the events on your SIEM solution directly."
- The SIEM integration settings do not replicate; set them on each node
  ([Bastion HA runbook](../runbooks/bastion-ha-replication.md), section 12).

Retention on the node:

- "The logs are also stored on the local file system" (Operation Guide 13.6): `syslog`,
  `wabaudit.log` and `wabauth.log` under `/var/log`.
- They are purged by "Remove user logs older than"; "The maximum retention time for the logs is
  365 days or 52 weeks" (Operation Guide 6.5.2).
- `WABJournalCtl`, run as `wabadmin`, displays the journal: "The logs include SIEM logs and system
  log for the OS." `--raw` displays a raw log file in `/var/log` (Operation Guide 13.3).
- **Audit > Authentication history** lists proxy logins
  ([Bastion 12.4.3 Auditor Guide](https://doc.wallix.com/) 10; see [section 2.1](#21-authentication-wabauth)).
- Audit tables stay on the node that wrote them (limitations table in the
  [Bastion HA runbook](../runbooks/bastion-ha-replication.md), section 1).

### 1.3 Access Manager log files and forwarding

Files ([Access Manager 6.0.5 Administration Guide](https://doc.wallix.com/) 9.7, 9.7.1):

- "The access log file (access.log) which records connections from external clients", "The
  technical log files (error.log, cli.log, and tech.log)" and the Java `hs_err_pid` file are
  "stored in the following directory: /var/log/wabam".
- Log levels and the archive download are on **Settings > Application Settings**, **Logs** tab.
  Switch to DEBUG before generating an archive. "The TRACE or ALL modes may expose sensitive
  information, including passwords."
- *Access Manager 5.2:* the path was `/var/log/wallix/wabam/{access,error,tech}.log`
  ([AM 5.2.4.0 Administration Guide](https://pam.wallix.one/documentation/admin-doc/am-admin-guide_en.pdf)).

Forwarding is contradictory in the 6.0.5 guides (gap A7):

- The Deployment Guide network table (2.2, Table 4) has a row "Syslog server integration", port
  "514/UDP", required when "Syslog message routing is enabled.", "Configurable in System > SIEM
  integration." ([Access Manager 6.0.5 Deployment Guide](https://doc.wallix.com/) 2.2). The
  Deployment Guide also suggests to "implement a SIEM solution so as not to lose any event logs"
  (2.4.1).
- The Administration Guide has no SIEM integration page. Its System chapter (8.4) covers
  Network, context path, time zone and NTP, SNMP and Service control, and it does not describe
  the message content.
- The table row is worded exactly as the Bastion 12.4.3 Deployment Guide row, so it may be a
  template carry-over (*inference*).
- An OS shipping agent is not an alternative: "The installation of external components such as
  Endpoint Detection and Response (EDR), backup, or monitoring agents is forbidden."
  ([Access Manager 6.0.5 Deployment Guide](https://doc.wallix.com/) 1.3, Administration Guide 1.3).

Until WALLIX confirms the forwarder, the only documented way to collect the files is the
archive download from the **Logs** tab (*inference*).

## 2. Bastion event format

Bastion messages carry a bracketed tag followed by key=value pairs
([Bastion 12.4.3 SIEM Logs Guide](https://doc.wallix.com/)). The Splunk sourcetype `WB:syslog`
extracts the tag as `WB_Event`; the add-on dashboards use `wabauth`, `sshproxy`, `rdpproxy`,
`SSH Session` and `RDP Session`. Tags in the guide:
...
Format and examples (SIEM Logs Guide 2.1; wrapped lines joined, typographic quotes
straightened):
...
- A login produces a `started` line (identification: `identified with {name}({TYPE})`), then a
  `success`, `failure` or `canceled` line. The status list says `cancel` and the example prints
  `canceled`; match both.
...
- Failures carry no reason ("Authentication failed"), except the lockout text. The reason for a
  refused second factor must come from Trustelem ([section 3](#3-trustelem-event-format)).
...
- The flow examples omit `type="UserAuth"` and may carry the program name as a prefix
  (`wabengine: [wabauth] ...` in the SSH flow, 6.9). The RDP flow (8.14) also shows a
  per-method line before the `started` line:
...
- **Audit > Authentication history** lists "the authentication attempts on the RDP and SSH proxy
  interfaces (respectively on ports 3389 and 22)". Each row has Timestamp, User name, Source IP,
  Result (a success or failure icon) and Diagnosis, which "provides more details on the
  authentication result"; the list filters by date range and text, and "All data on this page
  can be downloaded as a .csv file." "Authentication attempts with an expired OTP are not
  attributed to a user and are logged as [unknown username]." (Auditor Guide 10). It does not
  cover web interface logins.
...
(SIEM Logs Guide 3, quotes straightened). The examples also carry `client_ip`. Values in
`infos` are quoted lists, and edits show old and new values (`description ['' to 'DESCRIPTION']`).
Examples relevant to this design:
...
- Object types include Account, Answer, Apikey, Approval, AuthDomain (Auth domain),
  ...
  session, sessionlog, SiemLog, User, UserAuth (External authentication) and Usergroup.
- Actions include add, edit, delete, list, view, save, backup, download, restore, terminate,
  log, Unlock and Change.
- The types `Ldapdomain` and `LdapMapping` seen in older Sekoia samples do not appear in the
  12.0.25 or 12.4.3 guides; use `AuthDomain` and `AuthDomainMapping`.
...
- `duration` "follows the h:mm:ss format" (examples "0:00:07", "16:23:16", "157:45:59"). Parse
  it; do not compare it as a string.
- The Sekoia `[Vault Activity]` sample also carries `type="Vault"` and `vault="local"`, which
  the guide does not show.

## 3. Trustelem event format

The Trustelem records to parse are those of the API `Log` type: `id, date, level, msg, details,
userID, userEmail, ip, useragent`
([Trustelem API](https://trustelem-doc.wallix.com/books/trustelem-administration/page/api)). The
SIEM page does not document the JSON record fields; *inference:* they follow the `Log` type
([07 Operations](../trustelem/07-operations.md), section 2.2).

- Second-factor events (push accepted or rejected, TOTP, passkey) are forwarded with the other
  events, including `webauthn_duplicate_credential`
  ([chapter 06](../trustelem/06-mfa-and-access-rules.md)).
- The LDAP and RADIUS requests relayed by Trustelem Connect appear on the Trustelem Logs page:
  "Go on your Trustelm [sic] Logs page and see if you have LDAP or Radius logs. If yes, the
  connector is working and you should know why the authentication failed"
  ...
  Debug). They are the way to correlate a Bastion `wabauth` failure with a Trustelem decision
  (user not found, no access rule, no second factor). *Inference:* each record names the
  application; the `Log` type has no application field, so check a captured record.

## 4. Correlation keys

The user, client address, session and time fields below join the three sources. Keep NTP on
every node: SAML validation and correlation both depend on it.
...
The rules below use the fields of sections 2 to 4.
...
| MFA silently disabled | Bastion `wabaudit` `action="edit"` or `delete` on type `AuthDomain` or `UserAuth` (the RADIUS external authentication); the `infos` attribute that carries the secondary authentication is not documented (*gap* B6 in the [register](open-questions-and-gaps.md#3-bastion), capture in the lab); Trustelem permission change to *Always allow*: the Always allow event name is not documented, so compare nightly `listPerms` exports ([Trustelem API export reference](trustelem-api-export.md)) |
| Factor reset abuse | rescue code issued, factor reset, then login from a new IP address for the same user; rescue code requests appear on the Trustelem **Alerts** page ([Loss of a second factor](https://trustelem-doc.wallix.com/books/trustelem-administration/page/loss-of-a-second-factor)); the log wording of a factor reset is not documented (*inference:* match the Trustelem `msg` text of a captured record) |
...
| Connector outage | absence of Trustelem LDAP or RADIUS log records for more than 5 min during business hours while Bastion `wabauth` failures rise; the guide documents no timeout message, so a timeout is seen as a failure after the RADIUS timeout, *inference*: proxy `TIME_METRICS` `primary_authentication` durations grow. For ADConnect, the directory LED on the Trustelem dashboard ("the led indicates if the directory synchronization works, needs attention or doesn't work", [Trustelem summary](https://trustelem-doc.wallix.com/books/trustelem-administration/page/summary)); the LED is documented for directories only, not for Trustelem Connect |
| SAML certificate expiry | the Trustelem expiry alert e-mail ([07 Operations, section 4](../trustelem/07-operations.md#4-application-certificates-saml-signing)) and SAML errors in the Access Manager logs ([section 1.3](#13-access-manager-log-files-and-forwarding)) |
...
- The Splunk add-on's kill action calls
  `PUT https://<bastion>/api/sessions?session_id=<id>` with body `{"reason": "..."}` and headers
  `X-Auth-Key` and `X-Auth-User`.
- The helper hard-codes `X-Auth-User: admin`, so the key must be valid for that user.
- A kill is audited as `wabaudit` `action="terminate"` `type="sessionlog"` with
  "reason ['Killed by admin']" in `infos` (SIEM Logs Guide 3).
...
|---------|-----------|-------|
| Splunk | [WALLIX Splunk add-on](https://github.com/wallix/Splunk-add-on) | `[udp://514] sourcetype = WB:syslog` |
| Sekoia | [WALLIX Bastion intake](https://docs.sekoia.com/integration/categories/iam/wallix/) | ECS mapping: source.ip, destination.ip, user.name, event.action, event.outcome, process.command_line |
| Google SecOps | [WALLIX Bastion parser](https://docs.cloud.google.com/chronicle/docs/ingestion/default-parsers/wallix-bastion) | |
| FortiSIEM | [WALLIX Bastion configuration](https://docs.fortinet.com/document/fortisiem/7.6.0/external-systems-configuration-guide/717035/wallix-bastion) | |
| Cortex XSOAR | [WALLIX Bastion integration](https://xsoar.pan.dev/docs/reference/integrations/wallix-bastion) | |
