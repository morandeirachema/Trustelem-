...
| T5 | Exact SCIM payload, deprovisioning semantics, support for the WALLIX Bastion extension attributes; whether the five-minute reconciliation touches Bastion local users it did not create (break-glass, `am-auditor`) | 12 | vendor |
...
| # | Gap | Depends | Close by |
|---|-----|---------|----------|
| C1 | Licensing units and price: WALLIX Authenticator versus full WALLIX One IDaaS, Bastion per node or per user, Access Manager concurrent users, agents and SIEM push included or not, SMS cost, behaviour when the licence lapses | meeting blocks 2 | vendor |
