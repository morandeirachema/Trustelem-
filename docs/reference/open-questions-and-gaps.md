...
> - **Purpose:** one list of everything the sources do not answer, with the document that depends
>   on each item and the way to close it.
> - **Audience:** the PAM architect, the project lead preparing WALLIX meetings, and every editor
>   who marks a *gap* in a document.
> - **Verified:** 2026-09-24; every row re-checked against the Bastion 12.4.3 and 12.0.25 customer
>   guides (administration, deployment, operation, SIEM, user, auditor) and the Access Manager
>   6.0.5 customer guides (administration, deployment, user, auditor).
> - **Sources:** the citations in each row; customer guides are behind the
>   [doc.wallix.com](https://doc.wallix.com/) login.

## 1. How this register works

Each row is a question that the public sources and the customer guides leave open. The
**Depends** column names the chapter or document that relies on the answer. The **Close by**
column says who can answer it:

- *vendor*: needs a written WALLIX answer;
- *lab*: can be closed by a test;
- *console*: can be read in the Trustelem administration console or its script editor.

Maintenance rules:

1. Add a row when a document marks something "not documented" or *gap*.
2. Keep the ID for ever; other documents cite T1 to T16, B1 to B12, A1 to A7, S1 to S3 and
   C1 to C4.
3. When the answer is written into the document named in **Depends**, with its source, move the
   row to [section 7](#7-closed).

The customer guides behind the [doc.wallix.com](https://doc.wallix.com/) login were obtained on
2026-09-24 (Bastion 12.4.3 and 12.0.25; Access Manager 6.0.5), and the rows were re-checked
against them. The outbound form of this list is the
[vendor meeting script](vendor-meeting-script.md). The [sign-off questions](../architecture/08-caveats-and-questions.md#2-questions-to-put-to-wallix-before-sign-off) of the architecture set,
section 6 of the SCIM chapter and section 5 of the
[standards reference](standards-and-compliance.md) are per-document question lists that point
here.
...
| T11 | No WALLIX OIDC application template and no groups-claim guidance in the Trustelem books, so OIDC stays the alternative to SAML | [design decisions](../architecture/03-design-and-flows.md#1-design-decisions), [OIDC alternative](../architecture/03-design-and-flows.md#7-openid-connect-as-the-alternative-to-saml) | vendor |
| T12 | Behaviour when the Trustelem cloud is unreachable: no documented offline mode for Trustelem Connect (RADIUS, LDAP) or cached factors; break-glass relies on local Bastion and Access Manager accounts | [failure modes](../architecture/04-clusters-and-dr.md#3-failure-modes), 07 | vendor |
| T13 | Throughput and sizing figures for ADConnect and Trustelem Connect ("minimal resources" and two VMs each is all that is published) | [sizing](../architecture/05-low-level-design.md#5-sizing) | vendor |
...
| B6 | `wabauth` wording for a RADIUS second factor (success, failure, timeout), for API-key authentication, for Kerberos users and for logins brokered by Access Manager; the `wabaudit` `infos` attribute that carries the secondary authentication of an `AuthDomain` (needed by the "MFA silently disabled" detection rule); whether RADIUS accounting (1813) is used (no 12.4.3 guide mentions it). Closed parts, from the [Bastion 12.4.3 SIEM Logs Guide](https://doc.wallix.com/) 2.1: SAML, OIDC, local and LDAP lines; failures carry no reason ("Authentication failed") | logging, test plan | lab |
...
| B10 | Whether the RADIUS secondary authentication still runs after an SSH key or FIDO2 hardware-key login on the SSH proxy ([Bastion 12.4.3 Administration Guide](https://doc.wallix.com/) 2.5 and 7.2.5.6 document the key login, not its interaction with secondary authentication) | 04, [access paths](../architecture/03-design-and-flows.md#5-access-path-coverage) | lab, vendor |
| B11 | Trustelem MFA session behind a load balancer: the Bastion sends Framed-IP-Address so "RADIUS servers can use" it, but "when a load balancer sits in front of WALLIX Bastion, the address displayed is not the user's own" ([Bastion 12.4.3 Administration Guide](https://doc.wallix.com/) 7.2.5.4, 12.16.1.6); test whether the address Trustelem sees is the client's or the load balancer's, which would extend one user's MFA session to every source | 04, 06, [native client flow](../architecture/03-design-and-flows.md#3-identity-flow-native-client-path) | lab |
| B12 | Whether a Trustelem generic SAML2 application for the Bastion is needed when Access Manager fronts the Bastion, or whether the Access Manager application alone suffices (the Bastion never receives the assertion; *inference* in [chapter 04](../trustelem/04-bastion-integration.md)) | 04, SAML reference | vendor, lab |
...
## 6. Commercial and delivery

These are not documentation gaps, but only WALLIX can answer them. They are asked in the
...
Kept for the record. The answer is written into the document named in **Depends**.
