...
| B7 | Which Bastion domain the Access Manager SAML Domain Name must match. The [AM Admin Guide](https://pam.wallix.one/documentation/admin-doc/am-admin-guide_en.pdf) 10.3.2 and 10.5.2 and the [Bastion Admin Guide](https://pam.wallix.one/documentation/admin-doc/bastion_en_administration_guide.pdf) 7.3.1 name the *Domain server name* of the related authentication domain (Bastion recommends the same Authentication domain name); the Trustelem [Access Manager app page](https://trustelem-doc.wallix.com/books/trustelem-applications/page/wallix-access-manager) says "the Authentication domain name of your Active Directory Authentication domain", the Trustelem [Bastion SAML page](https://trustelem-doc.wallix.com/books/trustelem-applications/page/wallix-bastion-saml) says "AM Domain Name = Bastion Authentication domain name" of the SAML domain; this design sets both fields of a separate SAML *Other IdPs* domain to `TRUSTELEM` (report 7.3 step 7), which satisfies the product guides; the Active Directory reading stays open | 05, SAML reference | vendor, lab |
| B8 | Backup key length: the Deployment Guide says "at least 16 characters" in section 4.3 and "must be exactly 16 characters long" in chapters 6 and 7; the runbook uses 16 | runbook | vendor, lab |
